...
export class UploadEventClusterKeyword extends UploadClusterKeywordHelper {
  async execute(filePath: string) {
    ...
    return await prisma.$transaction(
      async (tx: Prisma.TransactionClient) => {
        await this.deleteClusterKeywords(KeywordType.EVENT);
...
const employeeClusterKeyword = new UploadEventClusterKeyword();
employeeClusterKeyword
  .execute("scripts/upload/EnglishEventClustersKeywords.csv")
