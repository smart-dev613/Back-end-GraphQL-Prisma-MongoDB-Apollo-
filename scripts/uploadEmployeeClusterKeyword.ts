...
import { KeywordType, Language, Prisma } from "@prisma/client";
...
export class UploadEmployeeClusterKeyword extends UploadClusterKeywordHelper {
  async execute(filePath: string) {
    ...
    return await prisma.$transaction(
      async (tx: Prisma.TransactionClient) => {
        await this.deleteClusterKeywords(KeywordType.EMPLOYEE);
...
const employeeClusterKeyword = new UploadEmployeeClusterKeyword();
employeeClusterKeyword
  .execute("scripts/upload/EnglishEmployeeClustersKeywords.csv")
